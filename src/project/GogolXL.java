 ...
 */
			...
			// this doesn't give us an optimal solution but at most one single Vertice left alone (a valid Pairings)
			.sorted( (p1,p2) ->  new Integer(p1.size()).compareTo(p2.size()))	
			
			.collect(Collectors.reducing(
							new ArrayList<Path>(), 		// initial container (guaranty a result)
							p -> Arrays.asList(p),		// path as list for aggregation
							(pathList,p2) -> { 			// mapper's arguments
					 			Path elem = p2.get(0); 	// because its a List (since its iterative, the second argument is always the single element's list )
